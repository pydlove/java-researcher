...
public class CommonProcessor {
    ...
    public static List<FieldInfo> doSampling(List<List<FieldInfo>> fixedGroups, int[] sampleCounts, int targetNum) {
    ...
    public static List<List<FieldInfo>> groupBySimilarityV1(
        ...
        for (FieldInfo fieldInfo : fieldInfos) {
            ...
            // 获取当前字段的向量表示
            INDArray currentVector = wordVectors.getWordVectorMatrixNormalized(combined);
            ...
            for (List<FieldInfo> group : groups) {
                ...
                // 获取组向量的表示
                INDArray groupVector = wordVectors.getWordVectorMatrixNormalized(groupCombined);
                if (groupVector == null) continue;

                // 计算余弦相似度
                double similarity = Transforms.cosineSim(currentVector, groupVector);
